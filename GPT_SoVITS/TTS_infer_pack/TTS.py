...
def set_seed(seed: int):
    ...
    try:
        ...
        try:
            import logging
            logging.getLogger("TTS").warning(f"torch.manual_seed failed: {e}")
        except Exception:
            print(f"Warning: torch.manual_seed failed: {e}")
    try:
        if torch.cuda.is_available():
            try:
                ...
                try:
                    import logging
                    logging.getLogger("TTS").warning(f"torch.cuda.manual_seed_all failed: {e}")
                except Exception:
                    print(f"Warning: torch.cuda.manual_seed_all failed: {e}")
...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config]):
        ...
        # 文本预处理器：传入回调以在BERT重载前后进行同步（注销/重新注册）
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        # 设置T2S模型内部模型的设备属性
        if hasattr(t2s_model.model, 'device'):
            t2s_model.model.device = self.configs.device
    ...
    def set_device(self, device: torch.device, save: bool = True):
        ...
        if self.t2s_model is not None:
            ...
            # 同步内部模型的设备属性，防止后续推理误用旧设备
            try:
                if hasattr(self.t2s_model, "model") and hasattr(self.t2s_model.model, "device"):
                    self.t2s_model.model.device = device
            except Exception:
                pass
        if self.vits_model is not None:
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        ...
        with torch.inference_mode():
            ...
            # 确保CNHuBERT模型与输入张量在同一设备上
            if self.cnhuhbert_model.model.device != self.configs.device:
                self.cnhuhbert_model.model = self.cnhuhbert_model.model.to(self.configs.device)
    ...
    def run(self, inputs: dict):
        ...
        try:
            ...
            for item in data:
                ...
                # 确保所有输入张量都在正确的设备上
                # all_phoneme_ids和all_bert_features是列表，需要检查列表中的每个张量
                all_phoneme_ids = [item.to(self.configs.device) if item.device != self.configs.device else item for item in all_phoneme_ids]
                all_phoneme_lens = all_phoneme_lens.to(self.configs.device) if all_phoneme_lens.device != self.configs.device else all_phoneme_lens
                all_bert_features = [item.to(self.configs.device) if item.device != self.configs.device else item for item in all_bert_features]
                
                # 模型已经在GPU上，无需检查设备状态（内存优化器已修复，不再移动模型）
            ...
            if not return_fragment:
                ...
                try:
                    # 清理音频张量
                    del audio
                    del batch_audio_fragment
                    del pred_semantic_list
                    del all_phoneme_ids
                    del all_bert_features
                    
                    # 清理临时变量
                    if 'all_pred_semantic' in locals():
                        del all_pred_semantic
                    if '_batch_phones' in locals():
                        del _batch_phones
                    if '_batch_audio_fragment' in locals():
                    ...
                    print(f"中间张量清理警告: {cleanup_error}")
            ...
            print("重置VITS模型...")
            if self.vits_model is not None:
                if hasattr(self.vits_model, 'cpu'):
                    self.vits_model.cpu()
                del self.vits_model
