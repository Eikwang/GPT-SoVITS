...
def cut5(inp):
    ...
    for item in mergeitems:
        if not set(item).issubset(punds):
            while len(item) > 50:
                split_index = 40
                while split_index > 0 and item[split_index] not in punds:
                    split_index -= 1
                if split_index == 0:
                    split_index = 35
                final_result.append(item[:split_index])
                item = item[split_index:]
...
if __name__ == '__main__':
    ...
    test_text = "你好，我是小明。你好，我是小红。你好，我是小刚。你好，我是小张。这是一段超过30个汉字的句子，需要进行切分处理。"
    print(method(test_text))