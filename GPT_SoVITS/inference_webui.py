...
cache = {}
cache_ttl = {}
CACHE_DEFAULT_MAX_ITEMS = 50
CACHE_DEFAULT_MAX_MB = 512  # 默认最大缓存占用（MB）
CACHE_DEFAULT_TTL_SEC = 300  # 默认缓存TTL（秒）
...
def clear_inference_cache():
    ...
    if cache_size > 0:
        ...
        print(f"已清理推理缓存，释放了 {cache_size} 个缓存项")
        return cache_size
    return 0

def get_cache_memory_usage():
    """获取缓存内存使用情况（仅估算Tensor占用）"""
    global cache
    total_size = 0
...
def manage_cache_size(max_cache_items=CACHE_DEFAULT_MAX_ITEMS, max_cache_mb=CACHE_DEFAULT_MAX_MB, ttl_seconds=CACHE_DEFAULT_TTL_SEC):
    """管理缓存大小，防止无限增长（支持TTL与内存预算）"""
    global cache, cache_ttl
    import time, gc
    now = time.time()
    ...
    if ttl_seconds is not None and ttl_seconds > 0:
        expired_keys = [k for k, ts in list(cache_ttl.items()) if (now - ts) > ttl_seconds]
        if expired_keys:
            for key in expired_keys:
                try:
                    if key in cache and hasattr(cache[key], 'cpu'):
                        cache[key] = cache[key].cpu()
                except Exception:
                    pass
                try:
            ...
            if torch.cuda.is_available():
                torch.cuda.empty_cache()
            print(f"缓存TTL清理，移除 {len(expired_keys)} 个过期项")

    # 计算当前占用
    def _current_usage_mb():
        return get_cache_memory_usage()

    # 若超过项数或内存预算，按时间顺序清理最旧项
    keys_sorted = list(cache_ttl.keys())
    # 确保按时间从旧到新排序
    try:
        keys_sorted.sort(key=lambda k: cache_ttl.get(k, 0))
    except Exception:
        pass

    cleaned = 0
    while True:
        over_items = (max_cache_items is not None and len(cache) > max_cache_items)
        over_memory = (max_cache_mb is not None and _current_usage_mb() > max_cache_mb)
        if not over_items and not over_memory:
            break
        if not keys_sorted:
            break
        key = keys_sorted.pop(0)
        try:
            if key in cache and hasattr(cache[key], 'cpu'):
                cache[key] = cache[key].cpu()
        except Exception:
    ...
    if cleaned > 0:
        ...
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        print(f"缓存大小/内存预算超限，已清理 {cleaned} 个旧缓存项")
    ...
    for i_text, text in enumerate(texts):
        ...
        if i_text in cache and if_freeze == True:
            ...
            with torch.inference_mode():
                ...
                cache_ttl[i_text] = __import__('time').time()
                
                # 管理缓存大小，防止内存泄漏
                manage_cache_size(max_cache_items=CACHE_DEFAULT_MAX_ITEMS, max_cache_mb=CACHE_DEFAULT_MAX_MB, ttl_seconds=CACHE_DEFAULT_TTL_SEC)
        t3 = ttime()
