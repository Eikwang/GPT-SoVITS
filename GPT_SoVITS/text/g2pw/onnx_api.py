...
class G2PWOnnxConverter:
    def __init__(
        ...
        # 确保配置文件路径存在
        config_file_path = os.path.join(uncompress_path, "config.py")
        if not os.path.exists(config_file_path):
            print(f"警告: 配置文件不存在: {config_file_path}")
            # 尝试使用默认配置
            self.config = load_config(config_path=None, use_default=True)
        else:
            self.config = load_config(config_path=config_file_path, use_default=True)
